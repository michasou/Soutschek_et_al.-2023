...
prepLFC4clust <- function( lfc, groups=NULL, signIncr=0.25, minLFC=0.15,
  ...
  if(any(rowSums(lfc != 0, na.rm=TRUE) == 0))
...
#' @export
plotClusterLFCs <- function(lfc, clust, summary=c("median","mean","smooth"), return.df=FALSE,ncol = NULL){
  summary <- match.arg(summary)
  if(!is.null(colnames(lfc))) 
  ...
  p <- ggplot(d, aes(class, val, group=feature)) + geom_line(alpha=0.3) + 
    facet_wrap(~cluster, scale="free_y",ncol = ncol) + ylab("") + xlab("")
  if(summary=="smooth"){
...
compoundPatternsDist <- function(sm, thres=0.1){
  tmp <- sapply(2:ncol(sm), FUN=function(x) sm[,x]-sm[,x-1])
  tmp <- sign(tmp)*(abs(tmp)>thres)
  sm1 <- sm-rowMedians(sm)
  sm1 <- sm1/quantile(abs(sm1),0.95)
  sm2 <- cbind(sm1, t(scale(t(sm))), tmp)
  2-cor(t(sm2),method="spearman")-cor(t(sm2))
}

getSmoothedMeans <- function(y,x,f=0.5,method=c("lowess","loess")){
  method <- match.arg(method)
  if(is.array(y)){
    sm <- t(apply(y,1,x=x,f=f,method=method,FUN=getSmoothedMeans))
    row.names(sm) <- row.names(y)
    colnames(sm) <- unique(x)
    return(sm)
  }
  if(method=="lowess"){
    y <- lowess(y~x, f = f)$y
  }else{
    mod <- loess(y~x, span=f)
    y <- predict(mod)
  }
  y[!duplicated(x)]
}
...
setcl <- function(x, cl){
  if(is(cl,"communities")){
    cl2 <- cl$membership
    names(cl2) <- cl$names
    cl <- cl2
  }
  if(is(cl,"kmeans")) cl <- cl$cluster
  if(is(cl,"pam")) cl <- cl$clustering
  cl <- cl[row.names(x)]
  rowData(x)$cluster <- factor(cl, levels=sort(unique(cl)))
  x
}
