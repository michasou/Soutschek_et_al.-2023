...
dea1229$gene_name <- sapply(strsplit(row.names(dea1229),"\\."),"[",2)
...
pdf("../Figure_Output/Heatmaps/Expl_1229_sig_Top30.pdf", width=8.5, height=6)
sechm::sechm(se2, deaTop30 , assayName ="corrected", top_annotation="diff", gaps_at="condition" ,breaks=TRUE, show_rownames = TRUE, anno_colors = ancols, 
             hmcols = heatcol, annocol_title_side=NA, do.scale = TRUE, sortRowsOn=NULL, cluster_rows=FALSE, 
             name = "scaled\nexpression", row_title = "pLNA-1229: Top 30 sig. genes")
dev.off()
```

```{r}
#Source data Fig. 4b
ll_RNA_4b <- as.data.frame(assays(se2)$corrected)
ll_RNA_4b <- ll_RNA_4b[deaTop30,]
write.csv(ll_RNA_4b,file = "../../../../12_SourceData/Fig4/Fig_4b.csv")
```

```{r}
#Source data, additional info
ll_info <- colData(se2)
write.csv(ll_info,file = "../../../../12_SourceData/Fig4/Fig_4_info.csv")
```
...
pdf("../Figure_Output/Heatmaps/Mitochondrial_Genes.pdf", width=7, height=5)
sechm::sechm(se2, MT_genes1, assayName ="corrected", top_annotation="diff", gaps_at="condition", breaks=FALSE, do.scale = TRUE, show_rownames = TRUE, anno_colors = ancols, hmcols = heatcol, annocol_title_side=NA, , name = "scaled\nexpression")
dev.off()
```

```{r}
#Source data Fig. 4d
ll_RNA_4d <- as.data.frame(assays(se2)$corrected)
ll_RNA_4d <- ll_RNA_4d[MT_genes1,]
write.csv(ll_RNA_4d,file = "../../../../12_SourceData/Fig4/Fig_4d.csv")
```
...
sechm::sechm(se2, ls , assayName ="log2FC.Ctrl", top_annotation="diff", gaps_at="condition", breaks=TRUE, show_rownames = TRUE, anno_colors = ancols, 
             name = "logFC", annocol_title_side=NA,
...
#TFAM targets
```{r}
collecTRI.regulons <- readRDS("../../data/collecTRI.regulons.rds")
...
sechm::sechm(se2, row.names(dea181c.sig), assayName ="corrected", top_annotation="diff", gaps_at="condition" ,breaks=TRUE, show_rownames = TRUE, 
             do.scale = TRUE, anno_colors = ancols, hmcols = heatcol, annocol_title_side=NA, name = "scaled\nexpression", 
...
sechm::sechm(se2, MC_genes_sig , assayName ="corrected", top_annotation="diff", gaps_at="condition", do.scale = TRUE, breaks=TRUE, 
             show_rownames = TRUE, anno_colors = ancols,name = "scaled\nexpression", 
             hmcols = heatcol, annorow_title_side = "left", annocol_title_side=NA, row_title = "MitoCarta 3.0 (pLNA-1229 FDR < 0.5)")
dev.off()
...
```{r}
findGO("cGAS")
dd <- getGOgenes("GO:0140896",species = "Hs")
dd <- dd$`cGAS/STING signaling pathway`
```

```{r}
png("../Figure_Output/Heatmaps/cGAS.png", width=9, height=7, units="in", res=300)
sechm::sechm(se2, dd , assayName ="log2FC.Ctrl", top_annotation="diff", gaps_at="condition" ,breaks=TRUE, show_rownames = TRUE, anno_colors = ancols, 
             hmcols = heatcol, annocol_title_side=NA, do.scale = FALSE,sortRowsOn=NULL, cluster_rows=FALSE, 
             name = "logFC", row_title = "Genes in the GO:0140896 - cGAS/STING signaling pathway")
dev.off()
```
