...
png("../Figure_Output/SingleGenes/Pink1.png", width=10, height=6, units="in", res=300)
...
```{r}
#manually curated inflammation genes
man <- c("IFNAR1","IFNAR2")  
g1 <- row.names(se)[rowData(se)$gene_name %in% man]

png("../Figure_Output/SingleGenes/Inflammation.png", width=12, height=6, units="in", res=300)
ggplot(meltSE(se, g1, assayName = "logcpm"), aes(condition, logcpm)) + geom_violin() + geom_point(aes(color = diff)) +
      theme_classic() + xlab("") +
