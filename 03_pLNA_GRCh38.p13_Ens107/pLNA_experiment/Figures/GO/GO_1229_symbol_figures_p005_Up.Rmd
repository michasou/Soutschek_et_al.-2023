...
#Import
```{r}
BP <- readRDS("../../longRNA_R/GO_Terms/GOdata/GO_UP_1229_BP_sig_P005_symbol_elim.rds")
```
...
dea1229_sig_down <- dea1229[dea1229$PValue < 0.05 & dea1229$logFC < 0,"Symbol"]
dea1229_sig_up <- dea1229[dea1229$PValue < 0.05 & dea1229$logFC > 0,"Symbol"]
```
...
Figure BP
```{r}
p1 <- ggplot(data = BP_Top, aes(x = as.factor(BP_Top$GO.ID))) +
  ...
  geom_label(label = BP_Top$label2, y = 22,label.size = 5, colour = NA, fill = "#636363") +
  geom_text(aes(label = round(BP_Top$Fold_Enrichment,2), y = 22), colour = "white") +
...
ggsave("../Figure_Output/GO-Terms/GO_1229_BP_ensembl_P005_UP.png", p1, width=11, height=4.75, units="in", dpi=300)
```
