...
```{r}
enr_1229_SC <- testEnrichment(dea1229,sets = sets_SC,tests = c("areamir","siteoverlap"), sets.properties = miRs)
...
```{r}
enr_3943_SC <- testEnrichment(dea3943,sets = sets_SC,tests = c("areamir","siteoverlap"), sets.properties = miRs)
res_3943_SC_ar <- getResults(enr_3943TS,test = c("areamir"),flatten = TRUE)
res_3943_SC_so <- getResults(enr_3943TS,test = c("siteoverlap.up"),flatten = TRUE)
```


# enrichMiR 181
```{r}
enr_181_SC <- testEnrichment(dea181c,sets = sets_SC,tests = c("areamir","siteoverlap"), sets.properties = miRs)
...
```{r}
#siteoverlap
p1 <- enrichPlot(res = res_1229_SC_so,enr.field = "enrichment",col.field = "props",sig.field = "FDR",label.field = "members", size.field = "overlap", label.sig.thres = 0.05)
...
```{r}
#siteoverlap
p3 <- enrichPlot(res = res_181_SC_so,enr.field = "enrichment",col.field = "props",sig.field = "FDR",label.field = "members", size.field = "overlap", label.sig.thres = 0.05)
...
title2 <- ggdraw() + draw_label("DEA miR-1229", fontface='italic', size = 18)
