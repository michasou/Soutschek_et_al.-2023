...
ag_s$norm <- ag_s$x / ag_s$Ctrl
```


#statistics
```{r}
mod_s1 <- rlm(x ~ sample + Diff,ag_s_mean)
summary(mod_s1)
...
ag_p$norm <- ag_p$x / ag_p$Ctrl
```


#statistics
```{r}
mod_s2 <- rlm(x ~ sample + Diff,ag_p_mean)
summary(mod_s2)
s2 <- emmeans(mod_s2, trt.vs.ctrl ~ sample , ref="pLNA-Ctrl")
s2
