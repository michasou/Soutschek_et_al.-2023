...
# statistics
```{r}
ag_area_stat <- aggregate(ag_area$x,ag_area[,c("Differentiation","Condition","Day")], FUN=mean)

mod_s1a <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_area_stat[ag_area_stat$Day == "Day 29",])
s1a <- emmeans(mod_s1a, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s1b <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_area_stat[ag_area_stat$Day == "Day 36",])
s1b <- emmeans(mod_s1b, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s1 <- rlm(x ~ Condition*Day + Differentiation*Day, psi = psi.huber, data = ag_area_stat)
...
ggsave2("../Figure_Output/SIM_MitoArea.png",p1, width = 6,height = 4.25, bg = "white")
```
...
ggsave2("../Figure_Output/SIM_MitoArea_Day36.pdf",p1b, width = 4.25,height = 4.75, bg = "white")
```
...
```{r}
#Source Data Fig. 5f
write.csv(ag_area[ag_area$Day == "Day 36",],file = "../../../../12_SourceData/Fig5/Fig_5f.csv")
```
...
```{r}
ag_length_stat <- aggregate(ag_length$x,ag_length[,c("Differentiation","Condition","Day")], FUN=mean)

mod_s2a <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_length_stat[ag_length_stat$Day == "Day 29",])
s2a <- emmeans(mod_s2a, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s2b <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_length_stat[ag_length_stat$Day == "Day 36",])
s2b <- emmeans(mod_s2b, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s2 <- rlm(x ~ Condition*Day + Differentiation*Day, psi = psi.huber, data = ag_length_stat)
...
s2a_df$p.value.signif <- round(s2a_df$p.value,3)
s2a_df$Day <- "Day 29"
...
ggsave2("../Figure_Output/SIM_MitoLength.png",p2, width = 6,height = 4.25, bg = "white")
```
...
```{r}
ag_inten_stat <- aggregate(ag_inten$x,ag_inten[,c("Differentiation","Condition","Day")], FUN=mean)
```

```{r}
mod_s3a <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_inten_stat[ag_inten_stat$Day == "Day 29",])
s3a <- emmeans(mod_s3a, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s3b <- rlm(x ~ Condition + Differentiation, psi = psi.huber, data = ag_inten_stat[ag_inten_stat$Day == "Day 36",])
s3b <- emmeans(mod_s3b, trt.vs.ctrl ~ Condition , ref="pLNA-Ctrl")

mod_s3 <- rlm(x ~ Condition*Day + Differentiation*Day, psi = psi.huber, data = ag_inten_stat)
...
s3a_df$p.value.signif <- round(s3a_df$p.value,3)
s3a_df$Day <- "Day 29"
...
ggsave2("../Figure_Output/SIM_MitoMeanIntensity.png",p3,width = 6,height = 4.25, bg = "white")
```

#####

## Intensity / Area correlation
```{r}
# p4a <- ggplot(data = e, aes(x = Mito_Area_um, y = Mito_MeanIntensity_SIM)) +
#   geom_point() + 
...
```{r}
# p4b <- ggplot(data = e, aes(x = Mito_Area_um, y = Mito_MeanIntensity_SIM)) +
#   geom_point() + 
