...
col_vec_black <- c("darkgrey","white","lightgrey","darkgrey")
...
#Aggregate Means
```{r}
#Synapses per DendArea (sqrt)
ag_s <- aggregate(sqrt(e$Synapses_perArea_um2),e[,c("Day","Differentiation","Condition")], FUN=mean)
...
#Syn per DendArea (sqrt)
ag_syn <- aggregate(sqrt(e$Syn_per_area_um2),e[,c("Day","Differentiation","Condition")], FUN=mean)
ag_syn$isEmpty <- ifelse(ag_syn$Condition == "Empty","p7","p8")
ag_syn21 <- ag_syn[ag_syn$Day == "Day21",]
ag_syn33 <- ag_syn[ag_syn$Day == "Day33",]

#PSD per DendArea (sqrt)
ag_psd <- aggregate(sqrt(e$PSD_per_area_um2),e[,c("Day","Differentiation","Condition")], FUN=mean)
...
st_df_mea <- st_df_mea[st_df_mea$group2 != "pLNA-3943",]
```
...
for(i in st_df_mea$Day) {
  ll <- sqrt(max(e[e$Day == i,"Synapses_perArea_um2"]))
  y_pos <- c(y_pos,ll)
}
...
```{r}
p1b <- ggplot(e,aes(x = Condition, y = sqrt(Synapses_perArea_um2))) + 
...
ggsave("../Figures/pLNA_Synapses.pdf",gt1b, width = 8,height = 5, bg = "white")
```

```{r}
#Source data Fig. 3e)
write.csv(e,file = "../../../../../12_SourceData/Fig3/Fig_3e.csv")
```
...
```{r}
# #black
# p1c <- ggplot(e,aes(x = Condition, y = sqrt(Synapses_perArea_um2))) + 
#   geom_violin(aes(colour = isEmpty2),fill = "black", show.legend = FALSE) +
#   stat_summary(
#     aes(color = isEmpty),
#     fun = mean,
#     geom="crossbar",
#     show.legend = FALSE
#   ) +
#   scale_color_manual(values = col_vec_black,guide = "none") +
#   new_scale_color() +
#   stat_summary(aes(color = Differentiation, alpha = isEmpty),
#     show.legend = TRUE,
#     fun = mean,
...
#         legend.position = "bottom",
#         legend.title = element_text(size = 16,margin = margin(0,20,0,0),color = "white"),
...
# g <- linesGrob(x = unit(c(.5, .5), "npc"),
#               y = unit(c(0.01,0.94), "npc"),
#               gp = gpar(col = "white", lty = "dotted", lwd = 1.5) )
# 
# 
# ## Add lines into the vertical gaps
...
# #p1c
# 
# ggsave("../Figures/pLNA_Synapses_black.png",gt1c, width = 8,height = 5, bg = "white")
# ggsave("../Figures/pLNA_Synapses_black.pdf",gt1c, width = 8,height = 5, bg = "white")
```
...
y_pos <- c()
for(i in st_df_mea$Day) {
  ll <- sqrt(max(e[e$Day == i,"Syn_per_area_um2"]))
  y_pos <- c(y_pos,ll)
}
...
p4c <- ggplot(e,aes(x = Condition, y = sqrt(Syn_per_area_um2))) + 
...
y_pos <- c()
for(i in st_df_mea$Day) {
  ll <- sqrt(max(e[e$Day == i,"PSD_per_area_um2"]))
  y_pos <- c(y_pos,ll)
}
...
p5 <- ggplot(e,aes(x = Condition, y = sqrt(PSD_per_area_um2))) + 
  geom_violin(aes(colour = isEmpty2),show.legend = FALSE) +
  stat_summary(
    aes(color = isEmpty),
    fun = mean,
...
psd <- psd[psd$Condition != "pLNA-3943",]
psd$Condition <- droplevels(psd$Condition)
psd <- psd[psd$Day != "Day27",]
psd$Day <- droplevels(psd$Day)
```
...
syn <- syn[syn$Condition != "pLNA-3943",]
syn$Condition <- droplevels(syn$Condition)
syn <- syn[syn$Day != "Day27",]
syn$Day <- droplevels(syn$Day)
```
...
p_syn <- ggplot(syn,aes(x = Condition, y = log(IntegratedIntensity_SYN))) + 
  ...
  scale_color_manual(values = col_vec2) +
  facet_wrap(~ Day) +
  ylab("Integrated Intensity SYN1 (log)") +
  theme_classic(base_size = 12)+
  theme(strip.text.x = element_text(size = rel(1)),
        strip.placement = "inside",
        text = element_text(size = 16), 
