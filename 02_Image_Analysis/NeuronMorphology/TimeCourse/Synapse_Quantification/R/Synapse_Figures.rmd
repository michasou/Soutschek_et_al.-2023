---
title: "Synpase_Figures"
author: "Michael"
date: "6 12 2020"
output: html_document
---


```{r}
library(ggplot2)
library(ggsci)
library(scales)
library(MASS)
library(emmeans)
library(rstatix)
```

# get data
```{r}
res <- readRDS("../R/Synapsecluster_Result.rds")
```

```{r}
replacement <- c(SC1 = "s1", SC2 = "s2", SC3 = "s3")
res$Replicate <- replacement[as.character(res$Replicate)]
res$Replicate <- factor(res$Replicate,levels = c("s1","s2","s3"))
```

# And some normalization
## Factor: 13.1359 px/micron
```{r}
#Soma Area
res$SomaArea_per_um <- res$AreaOccupied_AreaOccupied_Soma / (13.1359^2)

#Dendrite Area
res$DendArea_per_um <- res$AreaOccupied_AreaOccupied_DendMapTubeSomaRm / (13.1359^2)

#Length
res$Length <- res$Intensity_TotalIntensity_SkelSomaRm
res$Lenth_um <- res$Length / 13.1359

#PSD per micron
res$PSD_per_length_micron <- res$Count_PSDMaskByDEND / res$Lenth_um

#Syn Per micron
res$Syn_per_length_micron <- res$Count_SynMaskByDend / res$Lenth_um

#Co-Cluster per cell
res$Co_Cluster_perCell <- res$Count_MaskedSynByPSDdend / res$Count_MaskedNuclei

#Co-Cluster per area micron
res$Co_Cluster_perArea_px <- res$Count_MaskedSynByPSDdend / res$AreaOccupied_AreaOccupied_DendMapTubeSomaRm
res$Co_Cluster_perArea_um <- res$Co_Cluster_perArea_px * (13.1359^2)

#Co-Cluster per length
res$Co_Cluster_perLength_px <- res$Count_MaskedSynByPSDdend / res$Length
res$Co_Cluster_perLength_um <- res$Co_Cluster_perLength_px * (13.1359)



saveRDS(res,"../R/Synapsecluster_Result_with_anno.rds")
```


```{r}
res <- readRDS("../R/Synapsecluster_Result_with_anno.rds")
res$smooth <- "sm1"
```


```{r}
col <- "darkgrey"
col_vec <- c(pal_uchicago("default")(9)[1],pal_uchicago("default")(9)[3],pal_uchicago("default")(9)[2])
```



# PSD Density per Length
```{r}
ggplot(res,aes(x = as.factor(Day),y = PSD_per_length_micron * 10)) +
  geom_violin(show.legend = FALSE,adjust = 0.75) +
  stat_summary(
    fun = mean,
    geom="crossbar",
    show.legend = FALSE
  ) +
  stat_summary(aes(color = Replicate),
    show.legend = TRUE,
    fun = mean,
    fun.max = function(x) {mean(x) + sd(x)},
    fun.min = function(x) {mean(x) - sd(x)},
    geom="pointrange", position=position_dodge(width=0.5), size = 0.75
  ) +
  ylab("PSD95 Density [per 10micron]") +
  xlab("Day") +
  theme_classic(base_size = 13) +
  scale_color_uchicago()

ggsave("../Figures/PSD_Density.png", width = 7, height = 5)
```




# Syn Density per Length
```{r}
ggplot(res,aes(x = as.factor(Day),y = Syn_per_length_micron * 10)) +
  geom_violin(show.legend = FALSE,adjust = 0.75) +
  stat_summary(
    fun = mean,
    geom="crossbar",
    show.legend = FALSE
  ) +
  stat_summary(aes(color = Replicate),
    show.legend = TRUE,
    fun = mean,
    fun.max = function(x) {mean(x) + sd(x)},
    fun.min = function(x) {mean(x) - sd(x)},
    geom="pointrange", position=position_dodge(width=0.5), size = 0.75
  ) +
  ylab("SYN1 Density [per 10micron]") +
  xlab("Day") +
  theme_classic(base_size = 13) +
  scale_color_uchicago()

ggsave("../Figures/Syn_Density.png", width = 7, height = 5)
```




#paper

```{r}
ag_s <- aggregate(sqrt(res$Co_Cluster_perArea_um),res[,c("Day","Replicate")], FUN=mean)
```

```{r}
mod_s1 <- MASS::rlm(x ~ as.factor(Day) + Replicate ,ag_s,psi = psi.huber, maxit = 50)
summary(mod_s1)
s1 <- emmeans(mod_s1, pairwise ~ Day)


st_df1 <- as.data.frame(s1$contrasts)
st_df1$group1 <- sapply(strsplit(st_df1$contrast," - "),"[",1)
st_df1$group2 <- sapply(strsplit(st_df1$contrast," - "),"[",2)
st_df1$group1 <- gsub("Day","",st_df1$group1)
st_df1$group2 <- gsub("Day","",st_df1$group2)
st_df1 <- add_significance(st_df1,p.col = "p.value")

#show only statistical comparisons between neighboring time points
st_df_plot <- st_df1[c(1,5,8,10),]


#add y_position
y_pos <-sqrt(max(res[,"Co_Cluster_perArea_um"]))
st_df_plot $y_pos <- y_pos*1.05
st_df_plot$y_adj <- rep(c(0,0.05,0,0.05))
st_df_plot$y_pos2 <- st_df_plot$y_pos + st_df_plot$y_adj

ggplot(res,aes(x = as.factor(Day),y = sqrt(Co_Cluster_perArea_um))) +
  geom_smooth(stat="smooth",method = "loess", aes(group = smooth), color = col,
              alpha = 0.1, size = 0, show.legend = FALSE) +
  geom_line(stat="smooth",method = "loess", aes(group = smooth), color = col,
              alpha = 0.3, size = 1.5) +
  geom_violin(show.legend = FALSE,adjust = 0.75) +
  stat_summary(
    fun = mean,
    geom="crossbar",
    show.legend = FALSE
  ) +
  stat_summary(aes(color = Replicate),
    show.legend = TRUE,
    fun = mean,
    fun.max = function(x) {mean(x) + sd(x)},
    fun.min = function(x) {mean(x) - sd(x)},
    geom="pointrange", position=position_dodge(width=0.5), size = 0.75
  ) +
  ggsignif::geom_signif(
    data = st_df_plot,
    aes(xmin = group1, xmax = group2, annotations = p.value.signif, y_position = y_pos2),
    textsize = 4, vjust = -0.1, size = 0.75, manual = TRUE
  ) +
  ylab("Synapses per µm2 area (sqrt)") +
  xlab("Day") +
  scale_color_manual(values = col_vec)+
  theme_classic(base_size = 12)+
  scale_x_discrete(labels=c("Day 15","Day 21","Day 27","Day 33","Day 40")) +
  #coord_cartesian(ylim = c(NA,10)) +
  guides(color = guide_legend(title = "Differentiation")) +
  theme(legend.position = "bottom",
        text = element_text(size = 16), 
        axis.title.x=element_blank(),
        axis.text.x = element_text(size = rel(1)),
        axis.text.y = element_text(size = rel(1)),
        legend.text = element_text(size = 14),
        legend.title = element_text(size = 14))

ggsave("../Figures/Co_Cluster_overlay_mean_paper.png", width = 7, height = 5)
ggsave("../Figures/Co_Cluster_overlay_mean_paper.pdf", width = 7, height = 5.5)
#ggsave("../Figures/Co_Cluster_overlay_mean_paper_defense.png", width = 7, height = 4)
```


```{r}
#Source Data Fig. 1c
write.csv(res,file = "../../../../../12_SourceData/Fig1/Fig_1c.csv")
```



#paper black
```{r}
mod_s1 <- MASS::rlm(x ~ as.factor(Day) + Replicate ,ag_s,psi = psi.huber)
summary(mod_s1)
s1 <- emmeans(mod_s1, pairwise ~ Day)


st_df1 <- as.data.frame(s1$contrasts)
st_df1$group1 <- sapply(strsplit(st_df1$contrast," - "),"[",1)
st_df1$group2 <- sapply(strsplit(st_df1$contrast," - "),"[",2)
st_df1$group1 <- gsub("Day","",st_df1$group1)
st_df1$group2 <- gsub("Day","",st_df1$group2)
st_df1 <- add_significance(st_df1,p.col = "p.value")

#show only statistical comparisons between neighboring time points
st_df_plot <- st_df1[c(1,5,8,10),]


#add y_position
y_pos <-sqrt(max(res[,"Co_Cluster_perArea_um"]))
st_df_plot $y_pos <- y_pos*1.05
st_df_plot$y_adj <- rep(c(0,0.05,0,0.05))
st_df_plot$y_pos2 <- st_df_plot$y_pos + st_df_plot$y_adj


col <- "white"


p0 <- ggplot(res,aes(x = as.factor(Day),y = sqrt(Co_Cluster_perArea_um))) +
  geom_smooth(stat="smooth",method = "loess", aes(group = smooth), color = col,
              alpha = 0.1, size = 0, show.legend = FALSE) +
  geom_line(stat="smooth",method = "loess", aes(group = smooth), color = col,
              alpha = 0.3, size = 1.5) +
  geom_violin(show.legend = FALSE,adjust = 0.75, color = "white",fill = "transparent") +
  stat_summary(
    fun = mean,
    geom="crossbar",
    show.legend = FALSE,
    color = "white"
  ) +
  stat_summary(aes(color = Replicate),
    show.legend = TRUE,
    fun = mean,
    fun.max = function(x) {mean(x) + sd(x)},
    fun.min = function(x) {mean(x) - sd(x)},
    geom="pointrange", position=position_dodge(width=0.5), size = 0.75
  ) +
  ggsignif::geom_signif(
    data = st_df_plot,
    aes(xmin = group1, xmax = group2, annotations = p.value.signif, y_position = y_pos2),
    textsize = 4, vjust = -0.1, size = 0.75, manual = TRUE
  ) +
  ylab("Synapses per µm2 area (sqrt)") +
  xlab("Day") +
  scale_color_manual(values = col_vec)+
  theme_classic(base_size = 12)+
  scale_x_discrete(labels=c("Day 15","Day 21","Day 27","Day 33","Day 40")) +
  #coord_cartesian(ylim = c(NA,10)) +
  guides(color = guide_legend(title = "Differentiation")) +
  theme(strip.text.x = element_text(size = rel(1.1),color = "white"),
        strip.placement = "inside",
        text = element_text(size = 14),
        panel.background = element_rect(fill = "black", color = NA),
        plot.background = element_rect(fill = "black", color = NA),
        axis.title.x=element_blank(),
        axis.title = element_text(color = "white"),
        axis.text.x = element_text(size = rel(1),color = "white"),
        axis.text.y = element_text(size = rel(1),color = "white"),
        axis.text = element_text(color = "white"),
        axis.line = element_line(color = "white"),
        axis.ticks = element_line(color = "white"),
        strip.background = element_rect(fill = "transparent", color = "white"),
        strip.text = element_text(color = "white"),
        legend.text = element_text(color = "white"),
        legend.title = element_text(color = "white"),
        legend.background = element_rect(fill = "black", color = "white")
        )

ggsave("../Figures/Co_Cluster_overlay_mean_paper_black.png", width = 7, height = 5)
#ggsave("../Figures/Co_Cluster_overlay_mean_paper_defense_black.png", width = 8, height = 4.5)
```



#smooth defense black
```{r}
col2 <- pal_uchicago("default")(9)[2]

ggplot(res,aes(x = as.factor(Day),y = sqrt(Co_Cluster_perArea_um))) +
  geom_smooth(stat="smooth",method = "loess", aes(group = smooth), color = col2,
              alpha = 0.4, size = 0, show.legend = FALSE) +
  geom_line(stat="smooth",method = "loess", aes(group = smooth), color = "white",
              alpha = 0.3, size = 1.5) +
  ylab("Synapses per µm2 area (sqrt)") +
  xlab("Day") +
  scale_color_manual(values = col_vec)+
  theme_classic(base_size = 12)+
  scale_x_discrete(labels=c("Day 15","Day 21","Day 27","Day 33","Day 40")) +
  theme(strip.text.x = element_text(size = rel(1.1),color = "white"),
        strip.placement = "inside",
        text = element_text(size = 14),
        panel.background = element_rect(fill = "black", color = NA),
        plot.background = element_rect(fill = "black", color = NA),
        axis.title.x=element_blank(),
        axis.title = element_text(color = "white"),
        axis.text.x = element_text(size = rel(1),color = "white"),
        axis.text.y = element_text(size = rel(1),color = "white"),
        axis.text = element_text(color = "white"),
        axis.line = element_line(color = "white"),
        axis.ticks = element_line(color = "white"),
        strip.background = element_rect(fill = "transparent", color = "white"),
        strip.text = element_text(color = "white"),
        legend.text = element_text(color = "white"),
        legend.title = element_text(color = "white"),
        legend.background = element_rect(fill = "black", color = "white")
        )

ggsave("../Figures/Co_Cluster_smooth_defense_black.png", width = 7, height = 4)
#ggsave("../Figures/Co_Cluster_smooth_defense_black.pdf", width = 7, height = 4)
```


#smooth defense
```{r}
col2 <- pal_uchicago("default")(9)[2]

ggplot(res,aes(x = as.factor(Day),y = sqrt(Co_Cluster_perArea_um))) +
  geom_smooth(stat="smooth",method = "loess", aes(group = smooth), color = col2,
              alpha = 0.1, size = 0, show.legend = FALSE, level = 0.95) +
  geom_line(stat="smooth",method = "loess", aes(group = smooth), color = col2,
              alpha = 0.3, size = 1.5) +
  ylab("Synapses per µm2 area (sqrt)") +
  xlab("Day") +
  scale_color_manual(values = col_vec)+
  theme_classic(base_size = 12)+
  scale_x_discrete(labels=c("Day 15","Day 21","Day 27","Day 33","Day 40")) +
  theme(strip.text.x = element_text(size = rel(1.1)),
        strip.placement = "inside",
        text = element_text(size = 14), 
        axis.title.x=element_blank(),
        axis.text.x = element_text(size = rel(1)),
        axis.text.y = element_text(size = rel(1)))

ggsave("../Figures/Co_Cluster_smooth_defense.png", width = 7, height = 4.25)
ggsave("../Figures/Co_Cluster_smooth_defense.pdf", width = 7, height = 4.25)
```


```{r}
#Source data Fig. 3d)
write.csv(res,file = "../../../../../12_SourceData/Fig3/Fig_3d.csv")
```


