...
```{r}
#take out r2 as outlier, see TSB_Synapse_Figures.rmd
e <- e[e$Differentiation != "r2",]
e$Differentiation <- droplevels(e$Differentiation)
```
...
ag_syn <- aggregate(sqrt(e$Syn_per_area_um2),e[,c("Differentiation","Condition")], FUN=mean)

#PSD per DendArea (sqrt)
ag_psd <- aggregate(sqrt(e$PSD_per_area_um2),e[,c("Differentiation","Condition")], FUN=mean)
```
