...
#Load Data
```{r}
se <- readRDS("./SE.TX.salmon.raw.rds")
```
...
```{r}
tx2ge <- as.data.frame(rowData(se)) 
```
...
rowData(se2)[["DEA"]] <- cbind(dea2[row.names(se2),],tx2ge[row.names(se2),])
...
for(f in grep("condition", colnames(mm3), value=TRUE)){
  ...
  deas3[[f]] <- cbind(deas3[[f]][row.names(se2),],tx2ge[row.names(se2),])
...
rowData(se2)[["DEA.ind.any"]] <- cbind(deas3$any[row.names(se2),],tx2ge[row.names(se2),])
...
```{r}
saveRDS(se2,"./SE.dea.TX.salmon.raw.rds")
```
