...
#load data
```{r}
gsea_1229 <- readRDS("../../longRNA_R/GSEA/GSEA.DEA_10nM.1229.rds")
```
...
p1 <- ggplot(gsea_1229_top,mapping = aes(x = NES, y= reorder(pathways,-padj), color = padj,size = leadEdge_genes)) +
  ...
  ggtitle("GSEA: 10nM DEA miR-1229") +
  theme(plot.title = element_text(hjust = 0))

ggsave("../Figure_Output/GSEA/GSEA_10nM_1229.png",height = 8,width = 8)
```
...
#load data GO
```{r}
gsea_1229_GO <- readRDS("../../longRNA_R/GSEA/GSEA.DEA_10nM.1229_GOTerms.rds")
```
...
p2 <- ggplot(gsea_1229_GO_top,mapping = aes(x = NES, y= reorder(pathways,-padj), color = padj,size = leadEdge_genes)) +
  ...
  guides(size=guide_legend(title="# LeadingEdge Genes")) +
  scale_color_continuous(type= "viridis", guide=guide_colourbar(reverse = TRUE),name = "Adj. Pvalue") +
  ggtitle("GSEA (GO): 10nM DEA miR-1229") +
  theme(plot.title = element_text(hjust = 0))

ggsave("../Figure_Output/GSEA/GSEA_10nM_1229_GO.png",height = 7,width = 8)
```
