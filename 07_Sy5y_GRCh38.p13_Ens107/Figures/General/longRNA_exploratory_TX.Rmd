...
g1 <- row.names(se2)[rowData(se2)$gene_name %in% c("OPA1")]

g1 <- row.names(se)[rowData(se)$gene_name %in% c("OPA1")]

ggplot(meltSE(se2, g1, assayName = "logcpm"), aes(condition, logcpm)) + geom_violin() + geom_point(aes(color = experiment)) +  facet_wrap(~feature, scale="free") + theme_classic()
