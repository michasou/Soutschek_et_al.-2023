...
dea_plna <- read.csv("../../../03_pLNA_GRCh38.p13_Ens107/pLNA_experiment/Target_Analysis/miR1229/target_data/dea1229_targets_genebased.csv")
...
df$cond_name <- factor(df$cond_name,levels = c("Ctrl.","miR-1229 (10nM)","miR-1229 (20nM)"))
```
...
```{r}
# df2 <- meltSE(se_sy, row.names(se_sy), assayName = "logcpm")
# df2$gene_name <- factor(df2$gene_name,levels = Targets$gene_name)
# 
# #Mitochondrial Targets black
# p1b <- ggplot(df2[df2$gene_name %in% c("SYNJ2BP","ARL6IP1","MTX3","RTN1"),], aes(cond_name, logcpm)) + 
...
#         strip.text.y = element_text(size = rel(1.1),color = "white"),
#         strip.placement = "inside",
#         text = element_text(size = 14),
...
#         strip.background.y = element_rect(fill = "transparent", color = "white"),
...
# 
# 
# ggsave("../Figure_Output/Targets/MitochondrialTargets_black.png", plot = p1b, width = 10,height = 6, bg = "transparent")
# ggsave("../Figure_Output/Targets/MitochondrialTargets_black.pdf", plot = p1b, width = 10,height = 6, bg = "transparent")
```
...
sf$seqnames <- factor(sf$seqnames,levels = sf$seqnames)
```
...
for(j in gg){
  ...
  pa <- ggplot(dd, aes(cond_name, logcpm)) + geom_violin() + geom_point(aes(color = experiment)) +
      ...
      theme(text = element_text(size = 16), 
            ...
            axis.title.x=element_blank(),
            axis.text.x = element_text(size = rel(0.875)),
            axis.text.y = element_text(size = rel(0.875)))
  ...
  pab <- pab + theme(plot.margin = margin(t = -5, r = 0, b = -10, l = 0, unit = "pt"))
...
ppl <- plot_grid(NULL,l1,NULL,l2,NULL,nrow = 1)

pp <- plot_grid(pp,NULL,ppl,NULL,ncol = 1,rel_heights = c(1,-0.025,0.1,-0.025))

ggsave2("../Figure_Output/Targets/Targets_with_BS.pdf",pp, width = 9.5,height = 10.5, bg = "white")
```
...
#dosage
dd_sd <- df[df$gene_name %in% gg,]
write.csv(dd_sd,"../../../12_SourceData/Fig6/Fig_6f_left.csv")

#binding sites
mb_sd <- m2[m2$seqnames %in% gg,]
write.csv(mb_sd,"../../../12_SourceData/Fig6/Fig_6f_right_bindingsites.csv")

#transcript lengths
sf_sd <- sf[sf$seqnames %in% gg,]
write.csv(sf_sd,"../../../12_SourceData/Fig6/Fig_6f_right_txlength.csv")
```
