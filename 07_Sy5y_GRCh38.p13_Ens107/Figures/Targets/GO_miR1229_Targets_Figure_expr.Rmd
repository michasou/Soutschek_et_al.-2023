...
#Import
```{r}
CC <- readRDS("../../longRNA_R/Target_Analysis/GO_Term/GO_data/GO_CC_miR1229_Targets_10nM_expr_elim_005.rds")
MF <- readRDS("../../longRNA_R/Target_Analysis/GO_Term/GO_data/GO_MF_miR1229_Targets_10nM_expr_elim_005.rds")
BP <- readRDS("../../longRNA_R/Target_Analysis/GO_Term/GO_data/GO_BP_miR1229_Targets_10nM_expr_elim_005.rds")
```
...
p1 <- ggplot(data = CC_Top, aes(x = as.factor(CC_Top$GO.ID))) +
  ...
  geom_label(label = CC_Top$label2, y = 13.5,label.size = 5, colour = NA, fill = "#636363") +
  geom_text(aes(label = round(CC_Top$Fold_Enrichment,2), y = 13.5), colour = "white") +
  scale_y_continuous(limits = c(-2.25,15), name = "Number of Genes")+
  ...
  annotate("label", x = 10.9, y = 13.5,colour = NA,label = "Enrichment", fill = "#636363") +
  annotate("text", x = 10.9, y = 13.5,label= "Enrichment",colour = "white") +
  ggtitle("Genes with miR-1229 sites - Cellular Component") +
  theme_classic() +
  theme(text = element_text(size = 16),
        ...
        plot.margin = unit(c(3,1,1,3.68),"lines"),
        plot.title = element_text(vjust = 7.5))


ggsave("../Figure_Output/Targets/GO_CC_10nM_expr_Targets.pdf",p1, width=12, height=4.75, units="in", dpi=300)
```
...
write.csv(sd_df,"../../../12_SourceData/Fig6/Fig_6e.csv")
```
...
Figure
```{r}
p2 <- ggplot(data = GO, aes(x = as.factor(GO$GO.ID))) +
...
ggsave("../Figure_Output/Targets/GO_Combined_10nM_expr_Targets.png",plot = p2, width=12, height=6, units="in", dpi=300)
```
