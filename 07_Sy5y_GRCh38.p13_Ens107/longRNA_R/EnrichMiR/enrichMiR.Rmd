...
res_SC_so <- res_SC_so[!is.na(res_SC_so$members),]
```
...
#filter out miRNAs without family
res_SC_ind10_so<- res_SC_ind10_so[!is.na(res_SC_ind10_so$members),]
```
...
#filter out miRNAs without family
res_SC_ind20_so <- res_SC_ind20_so[!is.na(res_SC_ind20_so$members),]
```
...
res_SC_ind10_so_adapted <- res_SC_ind10_so
res_SC_ind10_so_adapted[2,"members"] <- gsub(";",";\n",res_SC_ind10_so_adapted[2,"members"])

p4a <- enrichPlot(res = res_SC_ind10_so_adapted,enr.field = "enrichment",col.field = "props",sig.field = "FDR",label.field = "members", size.field = "overlap", label.sig.thres = 0.001)
...
p4a <- p4a + scale_color_viridis_c(direction = 1) + theme_classic() + 
  ggtitle("EnrichMiR - siteoverlap") + labs(color="Expression") +
...
ggsave2("../../Figures/Figure_Output/EnrichMiR/miR1229_Enrichplot_SO_10nM.pdf",p4a, width = 6,height = 3.5, bg = "white")
```

```{r}
#Source data Fig. 6b)
write.csv(res_SC_ind10_so_adapted,"../../../12_SourceData/Fig6/Fig_6b.csv")
```
...
ggsave2("../../Figures/Figure_Output/EnrichMiR/miR1229_10nM_CDPlot_SC.pdf",p10, width = 5,height = 3.5, bg = "white")
```

```{r}
#Source data Fig. 6c)
write.csv(dea2,"../../../12_SourceData/Fig6/Fig_6c.csv")
```
