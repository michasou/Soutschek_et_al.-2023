...
#load data
```{r}
se_sal <- readRDS("../../RNA_GRCh38.p14_Ens112/data/SE.DEA.salmon.raw_ens112.rds")
...
se_sal$experiment <- replacement[as.character(se_sal$diff)]
se_sal$experiment  <- factor(se_sal$experiment ,levels = c("s1","s2","s3"))
se_sal$space <- "       "
```
...
colnames(ccp) <- gsub("SC","s",colnames(ccp))
colnames(ccp) <- sub("s(\\d+)_Day(\\d+)", "Day\\2 (\\1)", colnames(ccp))
...
colnames(ccs) <- gsub("SC","s",colnames(ccs))
colnames(ccs) <- sub("s(\\d+)_Day(\\d+)", "Day\\2 (\\1)", colnames(ccs))
...
### Paper main figure
```{r}
h1 <- Heatmap(ccsr_li, col = colors, cluster_rows = FALSE, cluster_columns = FALSE, name="Spearman rank", column_title="Spearman correlation rank", row_names_gp=gpar(fontsize=12), column_title_gp = gpar(fontsize = 14), row_title="Lin et al., Stem Cell Rep. 2021", heatmap_legend_param = list(direction = "horizontal"),
              show_column_names = TRUE, column_labels = se_sal$space)
```

```{r}
h2 <- Heatmap(ccsr_tre,col = colors, cluster_rows = FALSE, cluster_columns = FALSE, name="Spearman rank", column_title="Spearman correlation rank", row_names_gp=gpar(fontsize=12),column_title_gp = gpar(fontsize = 14), row_title= "Trevino et al., Cell, 2021", heatmap_legend_param = list(direction = "horizontal"),
              show_column_names = TRUE, column_labels = se_sal$space)
```
...
pdf("./FigureOutput/Main_Lin.pdf", width=4, height=6.25)
draw(h1,heatmap_legend_side = "bottom"
)
dev.off()
...
pdf("./FigureOutput/Main_Trevino.pdf", width=4.5, height=6.25)
draw(h2,heatmap_legend_side = "bottom")
dev.off()
```
...
```{r}
#Source data Fig. 1g
write.csv(ccsr_li,file = "../../../12_SourceData/Fig1/Fig_1g_left.csv")
write.csv(ccsr_tre,file = "../../../12_SourceData/Fig1/Fig_1g_right.csv")
```
