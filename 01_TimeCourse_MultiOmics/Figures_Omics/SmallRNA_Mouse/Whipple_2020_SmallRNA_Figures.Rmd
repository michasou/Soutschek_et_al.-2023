...
#As of igraph version 1.6.0. the knn.graph function has to be corrected:
```{r}
knn.graph <- function(d, k=5){
    ...
    igraph::graph_from_adjacency_matrix(d, "max", weighted=TRUE)
}
```
...
h.se <- readRDS("../../smallRNA/data/Small_RNA/smallRNA_oasis.DEA.SE.rds")
```
...
# 
# #plot heatmap
# crossHm(list(Mouse=m.se.cl, Human=h.se.cl.40), features = com.sig.h40, do.scale = TRUE, gaps_at = "day", breaks=TRUE, show_rownames = FALSE, hmcols = heatcol,anno_columns=c("day"),anno_colors = ancols2)
```
