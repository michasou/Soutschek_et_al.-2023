...
#Fold change with respect to neuronal stage
```{r}
...
```{r}
pdf("./Figure_Output/RNA_Prot_heat_all_top1000.pdf", width=11.5, height=6)
a
dev.off()
```
...
```{r}
#Source data Fig. 1e
ll_RNA <- as.data.frame(assays(se_RNA_agg)$log2FC.neur)
ll_RNA <- ll_RNA[sig.neu.1000,]
ll_RNA$type <- "RNA"

ll_Protein <- as.data.frame(assays(se_PROT_agg)$log2FC.neur)
ll_Protein <- ll_Protein[sig.neu.1000,]
ll_Protein$type <- "Protein"

ll <- rbind(ll_RNA,ll_Protein)
write.csv(ll,file = "../../../12_SourceData/Fig1/Fig_1e.csv")
```
