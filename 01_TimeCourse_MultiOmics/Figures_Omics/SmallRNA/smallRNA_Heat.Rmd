...
```{r}
a <- sechm(se_mi, sig_mi, top_annotation =c("day"), assayName = "log2FC.neur", gaps_at = "day", breaks=TRUE, show_rownames = FALSE, hmcols = heatcol, anno_colors = ancols, show_colnames = FALSE, column_title = "miRNA", annocol_title_side=NA, name = "logFC",column_title_gp = gpar(fontsize = 14))

png("Figure_Output/Heat/smallRNA_Heat_miRNAs_alldays_normDay9.png", width=7, height=6, units="in", res=300)
a
dev.off()

pdf("Figure_Output/Heat/smallRNA_Heat_miRNAs_alldays_normDay9.pdf", width=6.5, height=4.5)
a
dev.off()
...
```{r}
#Source data Fig. 2a
ll_RNA <- as.data.frame(assays(se_mi)$log2FC.neur)
ll_RNA <- ll_RNA[sig_mi,]
write.csv(ll_RNA,file = "../../../12_SourceData/Fig2/Fig_2a.csv")
```
