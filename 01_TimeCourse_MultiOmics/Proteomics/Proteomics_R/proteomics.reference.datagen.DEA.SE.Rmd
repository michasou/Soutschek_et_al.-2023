...
row.names(se) <- sapply(strsplit(row.names(se),";"),"[",1)
```
...
prot.dea <- function(se,data,use = NULL,name,model){
  ...
  resp$ID <- NULL
  resp$UniProtName <- NULL
  resp$F <- NULL
  resp$t <- NULL
  resp$B <- NULL
  colnames(resp)[which(colnames(resp)=="P.Value")] <- "PValue"
  colnames(resp)[which(colnames(resp)=="adj.P.Val")] <- "FDR"
