...
p1c <- ggplot(ag_s_norm, aes(x = Condition, y = Normalized)) +
  ...
  scale_alpha_discrete(range = c(0.3, 0.6),guide = "none") +
  facet_wrap(~ Day) +
  ylab("Synapses per µm2 area per cell (Normalized to Ctrl.)") +
...
ggsave("../Figures/pLNA_Synapses_normtoCtrl.png",gt1c, width = 8,height = 5.5, bg = "white")
```
...
p4c <- ggplot(e,aes(x = Condition, y = sqrt(Syn_per_area_um2_per_Nuclei))) + 
  ...
  facet_wrap(~ Day) +
  ylab("Dendritic Syn1 counts per µm2 area per cell (sqrt)") +
  theme_classic(base_size = 12)+
...
ggsave("../Figures/pLNA_SynDensity.png",gt4, width = 8,height = 6.5, bg = "white")
```
...
p5 <- ggplot(e,aes(x = Condition, y = sqrt(PSD_per_area_um2_per_Nuclei))) + 
  ...
  facet_wrap(~ Day) +
  ylab("Dendritic PSD counts per µm2 area per cell (sqrt)") +
  theme_classic(base_size = 12)+
...
ggsave("../Figures/pLNA_PSDDensity.png",gt5, width = 8,height = 6.5, bg = "white")
```
...
ggsave2("../Figures/pLNA_psd_IntegratedIntensity.png",p_psd, width = 8,height = 6.5, bg = "white")
```
...
ggsave2("../Figures/pLNA_syn_IntegratedIntensity.png",p_syn, width = 8,height = 6.5, bg = "white")
```
