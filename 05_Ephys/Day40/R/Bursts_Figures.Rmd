...
```{r}
# ggplot(e,aes(condition,sqrt(-e$`peak ampl. (pA)`))) +
#   geom_violin()
# 
...
agg_ch$DishID <- interaction(agg_ch$Differentiation, agg_ch$condition, drop = TRUE)
```


```{r}
# Test for Intraclass Correlation Coefficient (ICC)
icc_ch <- lmer(x ~ condition + Differentiation +  (1 | DishID), data = agg_ch)
vc <- VarCorr(icc_ch)
...
```{r}
#Source data Fig. 3j - right)
write.csv(agg_ch,file = "../../../12_SourceData/Fig3/Fig_3j_left.csv")
```
...
```{r}
#Source data Fig. 3j - left)
write.csv(agg_amp,file = "../../../12_SourceData/Fig3/Fig_3j_right.csv")
```
...
ggsave2("../Figures/Bursts_summary.pdf",pp, width = 7.8,height = 4.25, bg = "white")
```
