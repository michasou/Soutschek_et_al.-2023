...
resis$DishID <- interaction(resis$Differentiation, resis$condition, drop = TRUE)
```


```{r}
# Test for Intraclass Correlation Coefficient (ICC)
icc_resis <- lmer(input ~ condition + Differentiation +  (1 | DishID), data = resis)
vc <- VarCorr(icc_resis)
...
# ICC of about 0%, perform statistic on cellular level
