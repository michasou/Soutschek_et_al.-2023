...
## HIP
```{r}
hip <- read_excel("../HIP_Beregnet med Ctunder 30_ms.xlsx", 
    skip = 2)
hip$...8 <- NULL
```
...
```{r}
# Determine n
number_samples_hip <- hip_long %>%
  group_by(Diagnosis,Sex,miRNA) %>%
  summarise(
    number_n = n(),
  ) %>%
  ungroup()
...
ggsave("../FigureOutput/HIP.png",p1, width = 14,height = 9, bg = "white")
```
...
p2 <- p2 + plot_annotation(
  caption = "\nEfficiencies: 1229-3p - 108% / 125b-5p - 105% / 126-3p - 103% / 128-3p - 103% / 146a-5p - 105% / 191-5p - 103% / 210-3p - 107%",
  theme = theme(plot.caption = element_text(hjust = 1, size = 12,colour = "grey36",face = "italic"))
)

ggsave("../FigureOutput/PFC.png",p2, width = 14,height = 9, bg = "white")
```
