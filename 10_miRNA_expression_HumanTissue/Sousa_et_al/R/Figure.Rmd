...
#import table S6
```{r}
df <- read_excel("../data/aan3456-tabless1-s11.xlsx", 
...
```{r}
dfl$miRNA <- gsub("hsa-mir","hsa-miR",dfl$miRNA)
```
...
```{r}
dfl_filt$miRNA <- gsub("hsa-miR","hsa-\nmiR",dfl_filt$miRNA)
```
...
ggsave("../FigureOutput/FilteredRegions.png",p2, width = 9,height = 6, bg = "white")
ggsave("../FigureOutput/FilteredRegions.pdf",p2, width = 8,height = 5.5, bg = "white")
```


```{r}
#Source data Fig. 2e
write.csv(dfl_filt,file = "../../../12_SourceData/Fig2/Fig_2e.csv")
```
