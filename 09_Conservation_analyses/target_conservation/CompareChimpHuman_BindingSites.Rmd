...
```{r}
#Human
m1_1229 <- findSeedMatches(hsa_seq, hsa_1229, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
...
#Chimp
m2_1229 <- findSeedMatches(chi_seq, hsa_1229, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
...
#Check for correlation with DEA.pLNA-1229 logFC
```{r}
# se_tx <- readRDS("../../03_pLNA_GRCh38.p13_Ens107/pLNA_experiment/data/SE.dea.TX.salmon.raw.rds")
...
```{r}
#Human
m1_134 <- findSeedMatches(hsa_seq, hsa_134, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
...
#Chimp
m2_134 <- findSeedMatches(chi_seq, hsa_134, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
...
a <- cor.test(n$hsa_repression,n$chi_repression, method = "pearson")
a$p.value
a$estimate
...
for(i in names(hsa_exp)){
  hsa_scan <- hsa_exp[[i]]
  #human
  mh <- findSeedMatches(hsa_seq, hsa_scan, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
  ...
  #chimp
  mc <- findSeedMatches(chi_seq, hsa_scan, maxLogKd=NULL, BP = SnowParam(12, progressbar=TRUE), p3.extra =TRUE, ret="GRanges")
...
saveRDS(df,"miRNA_correlationvalues_humanchimp.rds")
```
...
#plot
```{r}
df <- readRDS("./miRNA_correlationvalues_humanchimp.rds")
df <- as.data.frame(df)
...
p3 <- ggplot(df,aes(x = cons_info, y = cor_R)) +
  ...
  scale_color_manual(values = col_vec[1]) + ylab("Correlation of predicted repression\nbetween human & chimp") + 
  xlab("") + labs(color = "") +
