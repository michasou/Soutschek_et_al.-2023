...
pdf("./Figure_Output/sequence_identity_expMiRs_transpose_ComplexHeatmap.pdf", width=6.5, height=4.5)
draw(
  Heatmap(t(e), col = colors, column_title  = "Homology to Human microRNA precursors",column_title_gp = gpar(fontsize = 14),name = "% homology", show_column_dend = FALSE, heatmap_legend_param = list(
        at = c(0,30,60,80,100), 
        labels = c("","not conserved", "60%", "80%","100%")
    )))
dev.off()
...
```{r}
#Source data Fig. 2b
ll_plot <- as.data.frame(t(e))
write.csv(ll_plot,file = "../../../12_SourceData/Fig2/Fig_2b.csv")
```
