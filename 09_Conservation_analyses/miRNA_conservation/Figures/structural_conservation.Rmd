...
se_smallRNA <- readRDS("../../../01_TimeCourse_MultiOmics/smallRNA/data/Small_RNA/smallRNA_oasis.DEA.SE.rds")
...
p_pairwise2 <- ggplot(df_plot_pairwise, aes(x = Mean.pairwise.identity, 
  ...
  geom_point(data = df_plot_pairwise[df_plot_pairwise$seed_mismatch == "TRUE",], aes(color = seed_mismatch), shape = 1, size = 4,
             show.legend = FALSE) +
  scale_color_manual(values = colors) +
  ggrepel::geom_text_repel(aes(label = ifelse(Structure.conservation.index < 0.975 | seed_mismatch == "TRUE", miRNA, "")), 
                           size = (12 / 2.845), show.legend = F, max.overlaps = 10, point.padding = 0.5) +
  ggtitle("Structural conservation pairwise versus human") +
  xlab("Mean pairwise identity")+
  ylab("Max. structure conservation index")+
  labs(color = "Seed Mutation") + 
...
ggsave("./Figure_Output/structural_conservation_pairwise.pdf",p_pairwise2, width = 7, height = 4.5)
```
...
```{r}
#Source data Fig. 2f
write.csv(df_plot_pairwise,file = "../../../12_SourceData/Fig2/Fig_2f.csv")
```
